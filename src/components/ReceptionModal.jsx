...
const ReceptionModal = ({ isOpen, onClose, centerId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 201 Created status bilan kelgan response'ni handle qilish
      if (res.status === 201 || res.status === 200) {
        const responseData = res.data?.data || res.data?.reception || res.data;
        console.log('Response data extracted:', responseData);
        
        if (responseData && (responseData.id || responseData.receptionId)) {
          setReceptionData(responseData);
          toast.success("Darsga yozildingiz, sizni kutamiz!");
        } else if (res.data?.message) {
          // API message qaytarsa
          toast.success(res.data.message);
          onClose();
        } else {
          // Response data yo'q bo'lsa ham success
          toast.success("Darsga yozildingiz, sizni kutamiz!");
          onClose();
        }
      } else {
